 		...
 		if (!empty($referer)) 
 		{
			...
			if (!$referer_is_local)
			{
				...
				if (preg_match('!('.join('|', $search_sites).')\.[^/]*/(?:(?:[^\?&]*(?:\?|&))+(?:'.join('|', $search_query).')=)([^&]*)!i', html_entity_decode($referer), $q))
				{
					...
					{
						$search = stripslashes(urldecode(preg_replace('/%u([\d\w]{4})/', '&#x$1;', htmlentities($q[2]))));
				...
				else if (strpos($referer, 'images.google.') !== false && preg_match('!prev=([^&]+)!i', html_entity_decode($referer), $p))
				{
					if (!empty($p[1]) && preg_match('!q=([^&]+)!i', urldecode($p[1]), $q))
					{
						...
						{
							$search = stripslashes(urldecode(preg_replace('/%u([\d\w]{4})/', '&#x$1;', htmlentities($q[1]))));
...
HERE;
	...
	function getHTML_PagesWatched() 
	{
		...
		if (!empty($watched)) 
		{
			...
			$where = "WHERE (`resource_checksum` = '".implode("' OR `resource_checksum` = '", $watched)."') AND `search_terms` = ''";
			// Ignore certain domains
			$ignoredDomains	= preg_split('/[\s,]+/', $this->prefs['ignoreReferringDomains']);
			if (!empty($ignoredDomains))
			{
				foreach ($ignoredDomains as $domain)
				{
					...
					$where .= ' AND `domain_checksum` != '.crc32($domain);
				}
			}
		}
		else
	...
	function getHTML_SearchesRecent() 
	{
		...
		$query = "SELECT `referer`, `search_terms`, `resource`, `resource_title`, `dt`, `img_search_found`
					FROM `{$this->Mint->db['tblPrefix']}visit` 
					WHERE
					`search_terms`!='' $filter
					ORDER BY `dt` DESC 
