...
class Mint
{
			...
			foreach ($this->cfg['pepperShaker'] as $pepperId=>$pepper) 
			{
				...
				if (@include_once(MINT_ROOT.$pepper['src']))
				{
					...
					else
					{
						...
						$error .= "<a href=\"#Install\" class=\"uninstall\" onclick=\"SI.Mint.uninstallPepper('missing {$pepper['class']}','$pepperId'); return false;\"><img src=\"app/styles/{$this->cfg['preferences']['style']}/images/btn-uninstall.png\" alt=\"Uninstall\" width=\"62\" height=\"22\" /></a>";
						$this->logError($error);
					}
				}
				else 
	...
	function _tidyDB() 
	{
		$this->logError('The _tidyDB() method has been eliminated in favor of the new _tidySave() method. Please report sighting this message on the Mint Forum : http://www.haveamint.com/forum/.');
	}
...
/*	function _gateway($action, $post = '')
	...
	function update() 
	{
		...
		if (empty($this->cfg['activationKey']) || $this->cfg['version'] < 200)
		{
			...
			{	
				...
				if ($result = $this->query($query))
				{
					if (mysql_num_rows($result))
					{
						...
						$rows = 0;
						$likePrefix = str_replace('_', '\_', $this->db['tblPrefix']);
						$query="SHOW TABLE STATUS LIKE 'pre120_{$likePrefix}visit'";
						if ($result = $this->query($query)) 
						{
		...
		// condense all updates below into one for final release version
		
		if ($this->cfg['version'] < 131)
		{
			$this->cfg['preferences']['singleColumn'] = false;
		}
		
		if ($this->cfg['version'] < 133)
		{
			$this->cfg['preferences']['stripeAll'] = false;
		}
		
		if ($this->cfg['version'] < 134)
		{
		...
		if ($this->cfg['version'] < 137)
		{
			$this->cfg['preferences']['style'] = 'vanilla_mint';
		}
		
		if ($this->cfg['version'] < 140)
		{
...
/*		$updatesAvailable = $this->_updatesAvailable();
		if ($updatesAvailable)
		{
			$update = "<a href=\"".$this->cfg['installDomain'].$this->cfg['installDir']."\" class=\"update-btn\"><span>Update".(($updatesAvailable > 1) ? 's' : '')." Available</span></a>";
	...
	function preferences() 
	{
		...
		foreach ($this->pepper as $pepperId=>$pepper) 
		{
			...
			{
				$html .= "\t<a href=\"#Uninstall\" class=\"uninstall\" onclick=\"SI.Mint.uninstallPepper('{$pepper->info['pepperName']}','$pepperId'); return false;\"><img src=\"app/styles/{$this->cfg['preferences']['style']}/images/btn-uninstall.png\" alt=\"Uninstall\" width=\"62\" height=\"22\" /></a>";
			}
		...
		if (!empty($uninstalledPepper))
		{
			foreach ($uninstalledPepper as $pathToPepper)
			{
				...
				if (isset($installPepper))
				{
					...
					if ($compatibility['isCompatible'])
					{
						$item .= "<a href=\"#Install\" class=\"install\" onclick=\"SI.Mint.installPepper('$pathToPepper'); return false;\"><img src=\"app/styles/{$this->cfg['preferences']['style']}/images/btn-install.png\" alt=\"Install\" width=\"62\" height=\"22\" /></a>";
					}
...
/*			$updatesAvailable = $this->_updatesAvailable();
			if ($updatesAvailable)
			{
				$html .= '<a href="'.$this->cfg['installDomain'].$this->cfg['installDir'].'" id="update-available"><span>Update'.(($updatesAvailable > 1) ? 's' : '').' Available</span></a>';
			} */
		}
		else
		{
	...
	function generateRSSLink($pepper_id, $title = '', $additional = array()) 
	{
		...
		return (!$this->isLoggedIn())?'':"<a href=\"{$this->cfg['installDir']}/?RSS=".md5($this->cfg['password'])."&amp;pepper=$pepper_id$add\" class=\"rss feed-btn\"$title><span>Feed</span></a>";
	}
	...
	function getFormattedFeedback()
	{
		...
		if (!empty($this->feedback['feedback']))
		{
			...
			{
				$html .= '<p><a href="'.$this->feedback['destination'].'"><img src="app/styles/'.$this->cfg['preferences']['style'].'/images/btn-'.$this->feedback['button'].'.png" width="62" height="22" alt="'.ucfirst($this->feedback['button']).'" /></a></p>';
			}
		}
	 ...
	 **************************************************************************/
	 function abbr($var, $len = 44, $reverse = false) 
	 {
		...
		else if (strlen($var) <= $len)// || $this->cfg['preferences']['singleColumn'])
		{
