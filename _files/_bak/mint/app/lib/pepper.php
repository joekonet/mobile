...
class Pepper
{
	...
	var $prefs		= array();
	var $data		= array();
	var $manifest	= array(); // an multi-dimensional array representing database tables and columns
	var $moderate	= array(); // an array of Pepper created tables whose size are not actively moderated by custom code, table must have an id and dt column
	...
	function Pepper($pepperId)
	{
		...
		if ($this->Mint->version < 200)
		{
			...
			$this->Mint->logErrorNote('<p>Mint has detected the unlicensed use of the Mint 2 Pepper class on a '.$this->Mint->getFormattedVersion().' installation. Mint 2 is a paid upgrade. Please login into the Mint Account Center and upgrade the license for this domain.</p>');
	...
	function getHTML_Graph($high = 0, $graphData = array())
	{
		...
		for ($i = 0; $i < $barCount; $i++)
		{
			...
			$uniqueHeight	= floor(260 * $barData[1] / $high);
			$totalMargin	= floor(260 * ($barData[0] - $barData[1]) / $high) - 5;
